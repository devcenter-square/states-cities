...
@mod_endpoints.route('/cities_with_state_name/<state_name>/', methods=['GET'])
def get_cities(state_name):
    lgas = LGA.get_all_cities_with_state_name(state_name.capitalize())
    return Response(dumps(lgas), mimetype='application/json')

@mod_endpoints.route('/cities_with_state_code/<state_code>/', methods=['GET'])
def get_cities_with_state_code(state_code):
    lgas = LGA.get_all_cities_with_state_code(state_code.upper())
    return Response(dumps(lgas), mimetype='application/json')
